...
ordenar_vuelos_por_hora_salida(N) :-
    findall(IdVuelo, vuelo(IdVuelo, N, _, _, _, _, _, _, _, _), VuelosSinOrdenar),
...
%% vuelos_dia(+D)
%
%  Informa los vuelos de un dia D, que pueden ser "LU", "MA", "MI", "JU",
%  "VI", "SA" o "DO". Los vuelos estan ordenados por horario de salida.
%
vuelos_dia(D) :-
	vuelo(_, NumeroVuelo, _, _, Frecuencia, _, _, _, _, _),
	member(D, Frecuencia),
	ordenar_vuelos_por_hora_salida(NumeroVuelo).
...
opciones :-
	Datos = [
		...
		['10', 'tiempo_vuelo_triangular(+N)']
	],
	Cabecera = ['Numero', 'Opciones'],
	cli_table(Datos, [head(Cabecera)]).


descripciones :-
	Datos = [
		['1', 'Busca las ciudades de destino dada una ciudad de origen O.'],
		['2', 'Busca las rutas que realiza un modelo de avion M, o un fabricante F.'],
		['3', 'Busca las rutas que puede realizar un modelo de avion A, teniendo en cuenta'],
		['', 'su rango de alcance y las distancia de las rutas.'],
		['4', 'Informa los vuelos de un dia D, que pueden ser "LU", "MA", "MI", "JU",'],
		['', '"VI", "SA" o "DO". Los vuelos estan ordenados por horario de salida.'],
		...
		['', 'un vuelo N. '],
		['8', 'Calcula la ganancia en dolares dado un vuelo N, el porcentaje de vendido del'],
		['', 'vuelo en clase turista T y bussines B. '],
		['9', 'Calcula la distancia dado el numero de un vuelo N, que realiza una ruta'],
		['', 'triangular. '],
		['10', 'Informa el tiempo total que demanda realizar un vuelo triangular N.']
	],
	Cabecera = ['Numero', 'Descripcion'],
	cli_table(Datos, [head(Cabecera)]).
